...
/** Load a IRIS template. */
void MatchingTemplate::loadIrisTemplate(char *fileName, int dataType)
{
	FILE *statFile = NULL;
	...
	else //if "*-template.mat" does not exists
	{
        ...
        createIrisTemplate(fileName, templateFileName.c_str(), dataType);
	}
}

/** Create a new IRIS template. */
void MatchingTemplate::createIrisTemplate(char *fileName,
                                          ...
                                          int dataType)
{
    ...
    for (int a=0; a<m_data[0].dim[0]; a++)
        for (int b=0; b<m_data[0].dim[1]; b++)
