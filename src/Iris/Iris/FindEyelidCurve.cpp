...
// xPupil is not used. need to check.
{
    ...
    // debug: show broken eye images
    //ImageUtility::showImage("left top eyelid", imgUtil->convertImageToIpl(leftTopEyelid));
    //ImageUtility::showImage("right top eyelid", imgUtil->convertImageToIpl(rightTopEyelid));
    //ImageUtility::showImage("left bottom eyelid", imgUtil->convertImageToIpl(leftBottomEyelid));
    //ImageUtility::showImage("right bottom eyelid", imgUtil->convertImageToIpl(rightBottomEyelid));
    //ImageUtility::showImage("center top eyelid", imgUtil->convertImageToIpl(centerTopEyelid));
    //ImageUtility::showImage("center bottom eyelid", imgUtil->convertImageToIpl(centerBottomEyelid));
    ////
...
double FindEyelidCurve::LagrangeInterpolation(double *x, double *f, int n, double xbar)
{
    ...
    for (int i=0; i<=n-1; i++)
    {
        ...
        for (int j=0; j<=n-1; j++)
             if (j != i)
                 l *= (xbar-x[j])/(x[i]-x[j]);  //lagrange mult
        sum += l*f[i];   // sum
    }
