...
void ImageUtility::showEyeLidPoints(const char* name, IplImage* img, double *x, double *ty, double *by)
{
    // this method is mainly for debug
    for (int i=0; i<3; i++) {
        std::cout << "x:[" << i << "] " << x[i] << "top ty:[" << i << "] " << ty[i] << std::endl;
        std::cout << "x:[" << i << "] " << x[i] << "top ty:[" << i << "] " << by[i] << std::endl;
    }
...
void ImageUtility::drawEyeLidLines(IplImage *eyeImg, int *x, int *ty, int *by, int size)
{
    IplImage* img = NULL;
...
IplImage* ImageUtility:: convertFilterToIpl(Masek::filter* filter)
{
    ...
    for(int j=0; j<grayImg->height; j++)
        for(int i=0; i<grayImg->width; i++)
            grayImg->imageData[i+j*grayImg->widthStep] = filter->data[i+j*filter->hsize[1]];
