							   ...
							   int gDataType, int pDataType)
{
    ...
    gallery->loadIrisTemplate(galleryName, gDataType);
    probe->loadIrisTemplate(probeName, pDataType);
...
int MatchAlg::oneEyeAnalysis(char *eyeFileName, int dataType)
{
    if(eyeFileName == NULL)
    {
        cout << "Failed to load the file name" << endl;
    ...
    MatchingTemplate *eyeTemplate = new MatchingTemplate();

    eyeTemplate->loadIrisTemplate(eyeFileName, dataType);
